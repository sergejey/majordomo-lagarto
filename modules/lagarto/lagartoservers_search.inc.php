...
*/
 global $session;
  ...
  // SEARCH RESULTS
  $res=SQLSelect("SELECT * FROM lagartoservers WHERE $qry ORDER BY ".$sortby_megaddevices);
  if ($res[0]['ID']) {
   colorizeArray($res);
   $total=count($res);
   for($i=0;$i<$total;$i++) {
    // some action for every record if required
    $properties=SQLSelect("SELECT * FROM lagartoendpoints WHERE SERVER_ID='".$res[$i]['ID']."' ORDER BY ENDPOINT_ID");
    if ($properties[0]['ID']) {
